                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade" id="profile-02" role="tabpanel" aria-labelledby="profile-02-tab">
                                <div class="card card-statistics">
                                    <div class="card-body">
                                        <form method="post" action="{{ route('upload_files') }}"
                                    ...
                                    <table class="table center-aligned-table mb-0 table table-hover"
                                        ...
                                        <tbody>
                                            @php
                                                $i = 1;
                                            @endphp
                                            @foreach ($Student->Images as $Images)
                                                <tr style='text-align:center;vertical-align:middle'>
                                                    <td>{{ $i++ }}</td>
                                                    <td>{{ $Images->file_name }}</td>
                                                    <td>{{ $Images->created_at }}</td>
                                                    <td colspan="2">
                                                        <a class="btn btn-outline-info btn-sm"
                                                            href="{{ url('downloadFile/' . $Student->Name . '/' . $Images->file_name) }}"
                                                            role="button"><i class="fas fa-download"></i>&nbsp;
                                                            {{ trans('Students_trans.Download') }}</a>

                                                        <button type="button" class="btn btn-outline-danger btn-sm"
                                                            data-toggle="modal"
                                                            data-target="#Delete_img{{ $Images->id }}"
